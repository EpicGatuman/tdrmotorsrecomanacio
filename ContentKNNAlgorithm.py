...
class ContentKNNAlgorithm(AlgoBase):
    ...
    def fit(self, trainset):
        ...
        # Computar la matriu de similitud entre ítems basat en contingut
        
        # Generar vectors per cada pel·lícula
        ml = MovieLens()
        genres = ml.getGenres()
        years = ml.getYears()
        mes = ml.getMiseEnScene()
        
        print("Computant la matriu de similitud per contingut...")
            
        # Computar la distància al pla entre gèneres per cada combinació de pel·lícules en una matriu 2x2
    ...
    def estimate(self, u, i):
        ...
        if not (self.trainset.knows_user(u) and self.trainset.knows_item(i)):
            raise PredictionImpossible("L'usuari o l'ítem són desconeguts")
        
        # Crear índex de similitud entre pel·lícula [i] i tots els elements que ha valorat l'usuari
        neighbors = []
        for rating in self.trainset.ur[u]:
            genreSimilarity = self.similarities[i,rating[0]]
            neighbors.append( (genreSimilarity, rating[1]) )
        
        # Extreure les valoracions més similars (top-k)
        k_neighbors = heapq.nlargest(self.k, neighbors, key=lambda t: t[0])
        
        # Calcular la mitjana ponderada dels top-k neighbours segons les valoracions de l'usuari
